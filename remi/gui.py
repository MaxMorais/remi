...
class HEAD(Tag):
    ...
    def add_javascript(self, content, key, _type = 'resource'):
        if _type == 'resource':
            self.add_child(key, f'<script type="application/javascript" src="{content}"></script')
        elif _type == 'source':
            self.add_child(key, f'<script type="application/javascript">{content}</script>')

    def set_internal_js(self, app_identifier, net_interface_ip, pending_messages_queue_length, websocket_timeout_timer_ms):
        self.add_javascript(
            'window.RemiSettings = {'
                f'host: "{net_interface_ip}",'
                f'maxPendingMessages: {pending_messages_queue_length},'
                f'messaginTimeout: {websocket_timeout_timer_ms}'
                f'emitterIdentifier: "{app_identifier}",'
                'event_name': "onerror"'
            '}',
            'settings_js',
            'source'
